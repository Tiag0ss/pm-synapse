...
export type CreateItemKind = 'note' | 'whiteboard';
...
interface CreateNoteModalProps {
  ...
  onConfirm: (
    title: string,
    opts: { kind: CreateItemKind; template?: SelectedTemplate }
  ) => void;
  /** Prefill title (e.g. create note from whiteboard text). */
  initialTitle?: string;
  /** Starting kind when the dialog opens. */
  initialKind?: CreateItemKind;
  /** Hide the Note | Whiteboard switch (create-from-text). */
  lockKind?: boolean;
}
...
export default function CreateNoteModal({
  ...
  useEffect(() => {
    if (!open) return;
    setItemKind(initialKind);
    setTitle(initialTitle);
    setFilter('');
    setError('');
    if (initialKind === 'whiteboard') {
      setTemplates([]);
      setSelectedId(null);
      setLoading(false);
      return;
  ...
  }, [open, initialTitle, initialKind, lockKind]);

  useEffect(() => {
    if (!open || itemKind !== 'note' || templates.length > 0 || loading) return;
    setLoading(true);
    void (async () => {
      try {
        const res = await fetch('/api/templates', { credentials: 'include' });
  ...
  }, [open, itemKind, templates.length, loading]);
  ...
  const canSubmit =
    Boolean(title.trim()) && (itemKind === 'whiteboard' || Boolean(selected));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <form
        ...
        onSubmit={(e) => {
          ...
          if (itemKind === 'whiteboard') {
            onConfirm(title.trim(), { kind: 'whiteboard' });
            return;
          }
          if (!selected) return;
          onConfirm(title.trim(), {
            kind: 'note',
            template: { id: selected.id, bodyMarkdown: selected.bodyMarkdown },
          });
        }}
      >
        ...
        {itemKind === 'note' ? (
          ...
          <div
            className="synapse-md-preview min-h-[160px] flex-1 overflow-auto p-5 text-[14px] leading-6"
            dangerouslySetInnerHTML={{ __html: previewHtml }}
          />
        )}
